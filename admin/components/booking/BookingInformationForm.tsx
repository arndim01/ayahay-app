...
export default function BookingInformationForm({
  ...
  const processPassengerData = (values: any) => {
    const bookingTrips = values.bookingTrips || [];
    
    bookingTrips.forEach((trip: any) => {
      const passengers = trip.bookingTripPassengers || [];
      
      passengers.forEach((passenger: any) => {
        // Ensure default values are included for disabled fields
        formPreferences?.forEach((pref) => {
          if (pref.defaultValue && !passenger.passenger[pref.field]) {
            passenger.passenger[pref.field] = pref.defaultValue;
          }
        });
      });
    });
  ...
  const validateFieldsInCurrentStep = async () => {
    ...
    try {
      ...
      // Process passenger data to include default values
      const processedValues = processPassengerData(values);
  ...
  const renderPassengerField = (
    ...
    // Always set the default value in the form regardless of field visibility
    if (defaultValue) {
      const fieldPath = [...name, fieldName];
      if (!form.isFieldTouched(fieldPath)) {
        setTimeout(() => {
          if (fieldName === 'sex') {
            const defaultSexValue = defaultValue.charAt(0).toUpperCase() + 
    ...
    // Create hidden form item for disabled fields
    if (!isEnabled) {
      return (
        <Form.Item 
          name={[...name, fieldName]}
    ...
    switch (fieldName) {
      ...
      case 'nationality':
        return (
          ...
          >
            <Input
              disabled={passengers?.[name[3]]?.passenger?.id > 0}
              placeholder={label}
      ...
      case 'dateOfBirth':
        return (
          <Form.Item
            ...
            colon={false}
            rules={
              required ? [{ required: true, message: `Missing ${label}` }] : []
            }
          >
            <DatePicker
              disabled={passengers?.[name[3]]?.passenger?.id > 0}
              ...
              defaultValue={
                getFieldDefaultValue(fieldName)
                  ? dayjs(getFieldDefaultValue(fieldName))
                  : undefined
              }
            />
          </Form.Item>
      ...
      case 'age':
        return hasPrivilegedAccess ? (
          <Form.Item name={[...name, fieldName]} label={label} colon={false}>
            <InputNumber
              disabled={passengers?.[name[3]]?.passenger?.id > 0}
              min={0}
              placeholder={label}
